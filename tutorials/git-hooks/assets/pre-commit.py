...
try:
    ...
    # Note: Could use -s instead of --porcelain but porcelain is guaranteed not to break
    ...
    for status_line in status_list:
        ...
        if status_line.startswith('A'):
            ...
            # git the relative path from Git in case the file is in a subdirectory of the working directory
            rpath = subprocess.check_output(
                ["git","rev-parse","--show-prefix"],
                stderr=subprocess.STDOUT).decode("ISO-8859-1")
            relative_path = rpath.rstrip()
            # construct the :<path> syntax needed for show to dump the contents of the staged version
            staged_fullpath = ":" + relative_path + staged_file
            # Use the git show :<path> syntax to get the contents of the actual staged version
